...
public class groupMangerServiceImpl implements IgroupMangerService {
		...
		try {
			...
			if (count > 0) {
				...
				if (rowCount > 0) {
					...
					// 写入项目表，发布广告改动时间数据
					projectMapper.updateParameterUpdateTimeByPrimaryKey(
							taxigroupMapper.selectByPrimaryKey(groupid).getProjectid(), d1.format(now));

					if (grpbelong == 0) {
						userMapper.updateGroupidsByname(adminname, "," + groupid);
					} else {
						userMapper.updateGroupidsByprojectid(projectid, "," + groupid);
					}
	...
	public JSONObject DeleteGroup(int grpid) {
		JSONObject jObject = new JSONObject();
		try {
			int terminalCount = terminalMapper.selectCountbygroupid(grpid);
			if (terminalCount != 0) {
				jObject.put("result", "fail");
				jObject.put("resultMessage", "分组中存在已注册的车辆不能删除,请先移除车辆!");
			} else {
				String projectid = taxigroupMapper.selectByPrimaryKey(grpid).getProjectid();
	...
	public JSONObject Set_parameter(String parameter) {
		...
		try {
			...
			case "brightness": {
				 ...
				 * "UTF-8")); if(!path.exists()) { jObject.put("result", "fail");
				 * jObject.put("resultMessage", "读取ini文件失败");
				 * 
				 * return jObject; }
				 * 
				 * File file = new File(URLDecoder.decode(path.getAbsolutePath(),
				 * "UTF-8"),"static/Area.ini"); if(!file.exists()) { jObject.put("result",
				 * "fail"); jObject.put("resultMessage", "读取ini文件失败");
				 * 
				 * return jObject; }
				 * 
				...
				String Longitude_latitude = ini.get(Jparameter.getString("ProvinceName"),
						Jparameter.getString("CityName"));
			...
			// 写入项目表，发布广告改动时间数据
			projectMapper.updateParameterUpdateTimeByPrimaryKey(
					taxigroupMapper.selectByPrimaryKey(Jparameter.getIntValue("groupid")).getProjectid(),
					d1.format(now));
	...
	public JSONObject Get_parameter(String parameter) {
		...
		try {
			...
			case "user": {
				...
				jObject.put("setDisconnectMode", Juser.getString("Playmode"));
			}
				;
				break;
			case "brightness": {
				...
				if (strBrightnessValueArray.length() > 0) {
					strBrightnessValueArray = strBrightnessValueArray.substring(0,
							strBrightnessValueArray.length() - 1);
				}
