...
public class getCode {
	...
	public List<byte[]> GetCodeListbyid(JSONObject jsoninfo, JSONObject jsonarritem) {
		try
		{
			GJ_codingCls cls=new GJ_codingCls(1000);
				...
				if(JArraybmStyle!=null && JArraybmStyle.size()>0)
				{
					for(int i=0;i<JArraybmStyle.size();i++)
					{
						...
						case 2:{//纯色图
							...
							BufferedImage image = new BufferedImage(basemapwidth, basemapheight, BufferedImage.TYPE_INT_RGB);
			...
			for(String dataKey : jsonarritem.keySet())   {  
				...
				for(int i=0;i<jArrpage.size();i++)
				{
					...
					{
					case 0/*图文*/:{
						BufferedImage bufferedImage = DrawTextGraphics.getImagebyJsonArray(JsonItemStyle.getIntValue("playtype"), JarrContext, jsonspecial, jitem.getInteger("width"), jitem.getInteger("height"));
							...
							for(int r=0;r<JarrContext.size();r++)
							{
								...
								for(int t=0;t<Jarrr.size();t++)
								{
									...
									{										
										...
										List<BufferedImage> bImages = grabberVideoFramer.GetFramers(upload + File.separator + fileName,jitem.getInteger("width"),jitem.getInteger("height"),st);
										
										BufferedImage bufImage = DrawTextGraphics.getBigbufImage(bImages,jitem.getInteger("height"));
										GJ_PictureAndTextcls andTextcls=new GJ_PictureAndTextcls();				
								...
								for(int t=0;t<Jarrr.size();t++)
								{
									...
									{										
										try
										{
											...
											if(w != jsonObject.getIntValue("width") || h != jsonObject.getIntValue("height"))
											{
			...
			List<byte[]> SendByteList =cls.GJ_AddAdScript(GJ_xieyiTypeEnum.GJ.getValue(), GJ_xieyiVersionEnum.one.getValue(), adscript);			
		...
		try
		{
			...
			if(itemlist!=null && itemlist.size()>0)
			{
				for(int i=0;i<itemlist.size();i++)
				{
					...
					{
					case 0/*图文*/:{
						BufferedImage bufferedImage = DrawTextGraphics.getImagebyJsonArray(JsonItemStyle.getIntValue("playtype"), JarrContext, jsonspecial, item.getItemwidth(), item.getItemheight());						
						if(bufferedImage != null)
						{
							GJ_PictureAndTextcls andTextcls=new GJ_PictureAndTextcls();				
							andTextcls.setmTextType(1);
							...
							for(int r=0;r<JarrContext.size();r++)
							{
								...
								for(int t=0;t<Jarrr.size();t++)
								{
									...
									{										
										...
										List<BufferedImage> bImages = grabberVideoFramer.GetFramers(upload + File.separator + fileName,item.getItemwidth(),item.getItemheight(),st);
										
										BufferedImage bufImage = DrawTextGraphics.getBigbufImage(bImages,item.getItemheight());
										GJ_PictureAndTextcls andTextcls=new GJ_PictureAndTextcls();				
										andTextcls.setmTextType(itemType);
										andTextcls.setmPicture(bufImage);
										m_texts.add(andTextcls);
	...
	public List<byte[]> GetplaylistCodeListbyid(int adid) {
		try {
			...
			List<byte[]> SendByteList = cls.GJ_AddPlayList(GJ_xieyiTypeEnum.GJ.getValue(), GJ_xieyiVersionEnum.one.getValue(), gj_Playlistcls);
	...
	//播放列表编码
	public List<byte[]> GetplaylistCodeListbyid(int level,int ScheduleType,int pubid,String quantums,String programlist) {
		try {
			//[{"infoid":442,"timelenght":10,"offsetlist":[37]},{"infoid":496,"timelenght":15,"offsetlist":[188]}]
			//{"advlist":[{"infoid":442,"timelenght":10,"offsetlist":[0]},{"infoid":496,"timelenght":15,"offsetlist":[10]}],"totalTimeLength":25,"lifeAct":"1999-09-09","lifeDie":"2019-03-02"}
		...
		{			
			...
			if(itemlist!=null && itemlist.size()>0)
			{
				for(int i=0;i<itemlist.size();i++)
				{
					...
					String Itembackcolor=item.getItembackcolor();
					Color m_fontbackcolor=null;
					if(Itembackcolor!=null && Itembackcolor!="")
					{
					...
					String Itemforecolor=item.getItemforecolor();
					Color m_fontforecolor=null;
					if(Itemforecolor!=null && Itemforecolor!="")
					{
						int colorfindex =   Integer.parseInt(Itemforecolor.substring(1),16);
						m_fontforecolor=new Color(colorfindex);
					}
					byte m_fontforetransparency=(byte)(item.getItemforeopacity() & 0xff);;
					*/
					JSONObject JsonItemStyle = JSONObject.parseObject(item.getItemstyle());					
					byte m_linkmove= (byte)(JsonItemStyle.getIntValue("linkmove")& 0xFF);
					byte m_looptime=(byte)(JsonItemStyle.getIntValue("looptime")& 0xFF);
					...
					*/
					...
					case 0/*图文*/:{		
						BufferedImage bufferedImage = DrawTextGraphics.getImagebyJsonArray(JsonItemStyle.getIntValue("playtype"), JarrContext, jsonspecial, item.getItemwidth(), item.getItemheight());
						//BufferedImage bufferedImage = getImagebyJsonArray(JarrContext,jsonspecial,item.getItemheight());
						if(bufferedImage != null)
						{
							GJ_PictureAndTextcls andTextcls=new GJ_PictureAndTextcls();				
							andTextcls.setmTextType(1);
							...
							for(int r=0;r<JarrContext.size();r++)
							{
								...
								for(int t=0;t<Jarrr.size();t++)
								{
									...
									{										
								        ...
								        //List<BufferedImage> bimgs = grabberVideoFramer.GetFramers(upload + File.separator + fileName,jsonObject.getIntValue("width"),jsonObject.getIntValue("height"),st);
										List<BufferedImage> bImages = grabberVideoFramer.GetFramers(upload + File.separator + fileName,item.getItemwidth(),item.getItemheight(),st);
										/*
										for(int b=0;b<bimgs.size();b++)
											{
												GJ_PictureAndTextcls andTextcls=new GJ_PictureAndTextcls();				
												andTextcls.setmTextType(itemType);
												andTextcls.setmPicture(bimgs.get(b));
												m_texts.add(andTextcls);
											}
										*/
										BufferedImage bufImage = DrawTextGraphics.getBigbufImage(bImages,item.getItemheight());
	...
	//图文变成图片
	/*
	private BufferedImage getImagebyJsonArray(JSONArray JarrContext,JSONObject jsonspecial,int height) {
		try {
			int width = 0;
					...
					{//图文混排数据
						...
						{				
							...
							if(jsonspecial!=null || !isIncode)
							{
								...
								BufferedImage bufImage = DrawTextGraphics.getbufImage(jsonObject.getString("value"),jsonObject.getString("fontName"),jsonObject.getIntValue("fontSize"),backcolor,forecolor,height,jsonspecial);
								
								imagelist.add(bufImage);
								width += bufImage.getWidth();
								//ImageIO.write(bufImage, "PNG", new File("E:/"+t+".png"));								
							}							
						};
						break;
						case 2:
						...
						{										
							...
							{							
								...
								width += bufnewImage.getWidth();
								//ImageIO.write(bufnewImage, "PNG", new File("E:/"+t+".png"));								
							}
							else {								
	...
	*/
		...
		try {
			...
			{					
				...
				for(int i=0;i<jArrayLoop.size();i++)
				{
					...
					if(lifeAct.equals("")){lifeAct="1999-09-09";}
					if(lifeDie.equals("")){lifeDie="2100-09-09";}
						...
						int ald = Integer.parseInt(jsonO.getString("lifeDie").replace("-", ""));;
						 ...
						 if(!(idie < ala || iact > ald))
						 {
							 ...
							 for(int z=0;z<arrayjs.getJSONObject(l).getJSONArray("advlist").size();z++)
							 {
								 ...
								 {
									 arrayjs.getJSONObject(l).getJSONArray("advlist").getJSONObject(z).getJSONArray("offsetlist").add(arrayjs.getJSONObject(l).getIntValue("totalTimeLength"));
									 int totalTimeLength = arrayjs.getJSONObject(l).getIntValue("totalTimeLength") + timelenght;
	...
	//播放列表编码
	public int GetplaylistCodeListbyid(int playlistSN,JSONObject playlist,int pubid,int level,String Timequantum,int groupid) {
		try {
			int MaxPackLength = taxigroupMapper.selectByPrimaryKey(groupid).getMaxPackLength();
			GJ_codingCls cls=new GJ_codingCls(MaxPackLength);
