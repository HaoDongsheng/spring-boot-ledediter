...
public class GJ_codingCls {
	...
	/// <returns></returns>
	public byte[] GJ_SendDataencoding(int type, int version, int mpageall, int mpage, int sendid, String key,
			byte[] bytedata) {
		try {
			...
			} else// 分多包
			{
				...
				for (int page = 0; page < pageAll; page++) {
					if ((page + 1) == pageAll)// 最后一包
					{
						...
						// System.arraycopy(bytedata, JSZ * page, myonebyte_1, 0,(byteinfoLen - JSZ *
						// page));
	...
	// 读图片---全彩2byte
	private byte[] picturetobyteIndex(BufferedImage map) {
		List<Integer> colorTabel = new ArrayList<Integer>();

		for (int w = 0; w < map.getWidth(); w++)// 列
		{
			for (int h = 0; h < map.getHeight(); h++)// 行
			{
				int luma = map.getRGB(w, h);
				if (!colorTabel.contains(luma)) {
					colorTabel.add(luma);
				}
			}
		}

		byte[] colorbyteTable = null;
		byte mapType = 8;
		// 生成颜色列表
		if (colorTabel.size() <= 2) {
			mapType = 5;
			colorbyteTable = new byte[2 * 3];
		} else if ((colorTabel.size() > 2) && (colorTabel.size() <= 4)) {
			mapType = 6;
			colorbyteTable = new byte[4 * 3];
		} else if ((colorTabel.size() > 4) && (colorTabel.size() <= 16)) {
			mapType = 7;
			colorbyteTable = new byte[16 * 3];
		} else if ((colorTabel.size() > 16) && (colorTabel.size() <= 256)) {
			mapType = 8;
			colorbyteTable = new byte[256 * 3];
		} else {
			mapType = 3;

			int group = (int) (map.getWidth() / 8);
			byte[] returnbyte = new byte[7 + map.getHeight() * map.getWidth() * 2];

			returnbyte[0] = mapType;// 图片类型
			returnbyte[1] = intTobyte(0);
			returnbyte[2] = intTobyte(0);

			returnbyte[3] = intTobyte(map.getWidth() / 256);// 图片宽高
			returnbyte[4] = intTobyte(map.getWidth() % 256);//
			returnbyte[5] = intTobyte(map.getHeight() / 256);//
			returnbyte[6] = intTobyte(map.getHeight() % 256);//
			int p = 7;
			for (int g = 0; g < group; g++) {
				for (int h = 0; h < map.getHeight(); h++)// 行
				{
					for (int l = 0; l < 8; l++)// 列
					{
						...
						returnbyte[p++] = intTobyte(val / 256);
						returnbyte[p++] = intTobyte(val % 256);
					}
				}
			}
		...
		for (int i = 0; i < colorTabel.size(); i++) {
			int R = ((colorTabel.get(i) & 0xff0000) >> 16);
			;
			int G = ((colorTabel.get(i) & 0xff00) >> 8);
			int B = (colorTabel.get(i) & 0xff);

			colorbyteTable[3 * i + 0] = intTobyte(R);
			colorbyteTable[3 * i + 1] = intTobyte(G);
			colorbyteTable[3 * i + 2] = intTobyte(B);
		}

		byte[] Index_map = new byte[map.getWidth() * map.getHeight()];
		int offset = 0;
		for (int x = 0; x < map.getWidth() / 8; x++)// 列
		{
			for (int y = 0; y < map.getHeight(); y++)// 行
			{
				for (int x1 = 0; x1 < 8; x1++) {

					int luma = map.getRGB(8 * x + x1, y);
					if (colorTabel.contains(luma)) {
						int color_index = colorTabel.indexOf(luma);
						Index_map[offset] = intTobyte(color_index);
						offset += 1;
					}
				}
			}
		...
		int[] Index_compress = null;
		switch (mapType) {
		case 5: {
			// 2位色
			Index_compress = new int[map.getWidth() * map.getHeight() / 8];
			for (int i = 0; i < Index_compress.length; i++) {
				int Temp = 0;
				for (int x = 0; x < 8; x++) {
					Temp >>= 1;
					if (Index_map[i * 8 + x] == 1) {
						Temp |= 0x80;
					}
				}
				Index_compress[i] = intTobyte(Temp);
			}
		}
			;
			break;
		case 6: {
			// 4位色
			Index_compress = new int[map.getWidth() * map.getHeight() / 4];
			for (int i = 0; i < Index_compress.length; i++) {
				byte temp1 = (byte) ((byte) Index_map[4 * i + 0] << 6);
				byte temp2 = (byte) ((byte) Index_map[4 * i + 1] << 4);
				byte temp3 = (byte) ((byte) Index_map[4 * i + 2] << 2);
				byte temp4 = (byte) ((byte) Index_map[4 * i + 3]);
				Index_compress[i] = (byte) (temp1 | temp2 | temp3 | temp4);
			}
		}
			;
			break;
		case 7: {
			// 16位色
			Index_compress = new int[map.getWidth() * map.getHeight() / 2];
			for (int i = 0; i < Index_compress.length; i++) {
				Index_compress[i] = (byte) ((Index_map[2 * i] << 4) | Index_map[2 * i + 1]);
			}
		}
			;
			break;
		case 8: {
			// 256位色
			Index_compress = new int[map.getWidth() * map.getHeight()];
			for (int i = 0; i < Index_compress.length; i++) {
				Index_compress[i] = (byte) Index_map[i];
			}
		}
			;
			break;
		...
		byte[] returnimg = new byte[7 + colorbyteTable.length + Index_compress.length];
		returnimg[0] = mapType;// 图片类型
		returnimg[1] = intTobyte(0);
		returnimg[2] = intTobyte(0);

		returnimg[3] = intTobyte(map.getWidth() / 256);// 图片宽高
		returnimg[4] = intTobyte(map.getWidth() % 256);//
		returnimg[5] = intTobyte(map.getHeight() / 256);//
		returnimg[6] = intTobyte(map.getHeight() % 256);//
		int index = 7;
		for (int i = 0; i < colorbyteTable.length; i++) {
			returnimg[index] = colorbyteTable[i];
			index += 1;
		}

		for (int i = 0; i < Index_compress.length; i++) {
			returnimg[index] = intTobyte(Index_compress[i]);
			index += 1;
		}
		return returnimg;
	}
	...
	private byte[] itemtobyte(GJ_ADitemcls aditem) {
		...
		case 0:// 图文
		{
			...
			for (int i = 0; i < aditem.getM_texts().size(); i++) {
						...
						for (int t = 0; t < tt.length; t++) {
							...
							if (tt[t] > 127)// 汉字
							{
								...
								} else// 非GB2312
								{
									...
									BufferedImage bmp = new BufferedImage(fontsize, fontsize,
									...
									FontDesignMetrics metrics = FontDesignMetrics
											.getMetrics(new Font("宋体", Font.PLAIN, fontsize));
				...
				case 1:// 图片
					/*
					 * 暂时改动只取3 textlist = new ArrayList<Byte>(); textlist.add(intTobyte(1));//图片类型
					 * textlist.add(intTobyte(0)); textlist.add(intTobyte(0));
					 * 
					 * textlist.add(intTobyte(aditem.getM_texts().get(i).getmPicture().getWidth() /
					 * 256));// 图片宽高
					 * textlist.add(intTobyte(aditem.getM_texts().get(i).getmPicture().getWidth() %
					 * 256));//
					 * textlist.add(intTobyte(aditem.getM_texts().get(i).getmPicture().getHeight() /
					 * 256));//
					 * textlist.add(intTobyte(aditem.getM_texts().get(i).getmPicture().getHeight() %
					 * 256));// byteArrayInsList(textlist,
					 * picturetobyte(aditem.getM_texts().get(i).getmPicture())); ptcount++;//图文加一
					 * mylist.addAll(textlist); textlist = new ArrayList<Byte>();//清空
					 * 
					 * break;
					 */
				case 2:// 带灰度图 8个像素点为一个字节
					 ...
					 * 暂时改动只取3
					 * 
					 * textlist = new ArrayList<Byte>(); textlist.add(intTobyte(2));//图片类型
					 * textlist.add(intTobyte(0)); textlist.add(intTobyte(0));
					 * 
					 * textlist.add(intTobyte(aditem.getM_texts().get(i).getmPicture().getWidth() /
					 * 256));// 图片宽高
					 ...
					 * textlist.add(intTobyte(aditem.getM_texts().get(i).getmPicture().getHeight() %
					 * 256));//
					 * byteArrayInsList(textlist,picturetobyteCUNSE(aditem.getM_texts().get(i).
					 * getmPicture())); ptcount++;//图文加一 mylist.addAll(textlist); textlist = new
					 * ArrayList<Byte>();//清空 break;
					 */
				case 3:// 全彩图片RGB2byte
					textlist = new ArrayList<Byte>();
					/*
					 * textlist.add(intTobyte(3));//图片类型 textlist.add(intTobyte(0));
					 * textlist.add(intTobyte(0));
					 * 
					 * textlist.add(intTobyte(aditem.getM_texts().get(i).getmPicture().getWidth() /
					 * 256));// 图片宽高
					 ...
					 * textlist.add(intTobyte(aditem.getM_texts().get(i).getmPicture().getHeight() %
					 * 256));//
					 * byteArrayInsList(textlist,picturetobyte2byte(aditem.getM_texts().get(i).
					 * getmPicture()));
					 */
					byteArrayInsList(textlist, picturetobyteIndex(aditem.getM_texts().get(i).getmPicture()));
	...
	public GJ_LedDecodingCls GJ_Decoding(byte[] ReceiveByte) {
		...
		try {
			if (ReceiveByte.length < 24) {
				return null;
			} // 长度太短不解析
			if ((ReceiveByte[0] != 0x7E) || (ReceiveByte[ReceiveByte.length - 1] != 0x7E)) {
				return null;
			} // 不符合协议结构，7E-7E
			...
			case "QSTR":// 智能顶灯查询星级
				...
				break;
			default:
				if (ReceiveByte.length < 61) {
					return null;
				} // 长度太短不解析
				if ((ReceiveByte[2] != 0x47) || (ReceiveByte[3] != 0x4A)
						|| (ReceiveByte[ReceiveByte.length - 3] != 0x45)
						|| (ReceiveByte[ReceiveByte.length - 2] != 0x4E)) {
					return null;
				} // 不符合协议结构，GJ-EN

				int alldatalen = (int) (ReceiveByte[4]) * 256 + (int) (ReceiveByte[5]);
				if (alldatalen != ReceiveByte.length - 3) {
					return null;
				} // 数据长度错误
				GJ_CRCcls crc = new GJ_CRCcls();// crc校验
				int crcvalue = crc.CRC_16(ReceiveByte, 4, ReceiveByte.length - 6);
				int crcvalue1 = (int) ((int) (ReceiveByte[ReceiveByte.length - 5]) * 256
						+ (int) (ReceiveByte[ReceiveByte.length - 4]));// crc
				if (crcvalue != crcvalue1) {
					return null;
				} // crc校验错
				myRcls.setMdataflows(ReceiveByte[12]);// 数据流向
				if (myRcls.getMdataflows() != 0x0B) {
					return null;
				} // 不是上行数据，短路等
				...
				stime = String.format("%02", (int) hh) + ":" + String.format("%02", (int) mm) + ":"
						+ String.format("%02", (int) ss);
				...
				stime = String.format("%02", (int) hh) + ":" + String.format("%02", (int) mm) + ":"
						+ String.format("%02", (int) ss);
				int fff = (int) (ReceiveByte[ReceiveByte.length - 19]) * 256
						+ (int) (ReceiveByte[ReceiveByte.length - 18]);
			...
			case qlst:// 查询播放列表
				...
				for (int i = 0; i < listlen; i++) {
					myRcls.getMreceivelistid()[i] = (int) (ReceiveByte[31 + i * 2]) * 256
							+ (int) (ReceiveByte[31 + i * 2 + 1]);
				}
				break;
			case qadv:// 查询广告列表
				...
				for (int i = 0; i < listlen; i++) {
					...
					myRcls.getMreceiveAdlist()[i]
							.setMid((int) (ReceiveByte[31 + i * 4]) * 256 + (int) (ReceiveByte[31 + i * 4 + 1]));
					myRcls.getMreceiveAdlist()[i]
							.setMcrc((int) (ReceiveByte[31 + i * 4 + 2]) * 256 + (int) (ReceiveByte[31 + i * 4 + 3]));
				}
				break;
			case dadv:// 删除广告
				break;
			...
			case TIME:
				// 年月日
				String strtime = ((int) (ReceiveByte[29]) + 2000) + "-" + String.format("%02", (int) ReceiveByte[30])
						+ "-" + String.format("%02", (int) ReceiveByte[31]) + " ";
				// 时分秒
				strtime += String.format("%02", (int) ReceiveByte[32]) + ":"
						+ String.format("%02", (int) ReceiveByte[33]) + ":"
						+ String.format("%02", (int) ReceiveByte[34]);
			...
			case qset:// 查询屏参
				...
				if (myRcls.getmSetLed().getM_setBrightnessMode() == 0) {
					...
					for (int i = 0; i < 8; i++) {
						...
						myRcls.getmSetLed().getM_BrightnessTimeArray()[i] = String.format("%02", (int) (timL / 60))
								+ ":" + String.format("%02", (int) (timL % 60));
					}
					for (int i = 0; i < 8; i++) {
				...
				if (myRcls.getmSetLed().getM_setZhaoMingMode() == 0) {
					...
					myRcls.getmSetLed().setM_ZhaoMingTimeStart(
							String.format("%02", (int) (timL / 60)) + ":" + String.format("%02", (int) (timL % 60)));
					timL = (int) (ReceiveByte[p++]) * 256 + (int) (ReceiveByte[p++]);
					myRcls.getmSetLed().setM_ZhaoMingTimeEnd(
							String.format("%02", (int) (timL / 60)) + ":" + String.format("%02", (int) (timL % 60)));
				} else {
					p += 4;
				}
			...
			case qss2:
				...
				timL = (int) (ReceiveByte[p++]) * 256 + (int) (ReceiveByte[p++]);
				myRcls.getmSet2().set_ZhaoMingTimeStart(
						String.format("%02", (int) (timL / 60)) + ":" + String.format("%02", (int) (timL % 60)));
				timL = (int) (ReceiveByte[p++]) * 256 + (int) (ReceiveByte[p++]);
				myRcls.getmSet2().set_ZhaoMingTimeEnd(
						String.format("%02", (int) (timL / 60)) + ":" + String.format("%02", (int) (timL % 60)));
				break;
			case qss3:
				p = 29;
				...
				for (int i = 0; i < 8; i++) {
					timL = (int) (ReceiveByte[p++]) * 10;
					myRcls.getmSet3().get_BrightnessTimeArray()[i] = String.format("%02", (int) (timL / 60)) + ":"
							+ String.format("%02", (int) (timL % 60));
				}
				for (int i = 0; i < 8; i++) {
