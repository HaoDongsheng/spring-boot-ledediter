...
public interface groupMapper {
	@Delete({ "delete from t_group", "where groupid = #{groupid,jdbcType=INTEGER}" })
	...
	@Select({ "select", "UpdateRate", "from t_group", "where groupid = #{0}" })
	@Result(column = "UpdateRate", property = "UpdateRate", jdbcType = JdbcType.DOUBLE)

	double selectUpdateRateBygroupid(Integer groupid);

	@Select({ "select", "pubid", "from t_group", "where groupid = #{groupid,jdbcType=INTEGER}" })
	@Result(column = "pubid", property = "pubid", jdbcType = JdbcType.INTEGER)
	int selectpubidBygroupid(Integer groupid);

	@Select({ "select", "plpubid", "from t_group", "where groupid = #{groupid,jdbcType=INTEGER}" })
	@Result(column = "pubid", property = "pubid", jdbcType = JdbcType.INTEGER)

	int selectplpubidBygroupid(Integer groupid);

	@Select({ "select", "count(*)", "from t_group", "where groupName = #{0} and DelIndex=0" })
	...
	@Update({ "update t_group", "set plpubid = #{plpubid,jdbcType=INTEGER}",
			"where groupid = #{groupid,jdbcType=INTEGER}" })
