...
public interface infocodeMapper {
    @Delete({
        "delete from t_infocode",
        "where infoSN = #{infoSN,jdbcType=INTEGER}"
    })
    int deleteByinfoSN(Integer infoSN);

    @Insert({
        "insert into t_infocode (infoCodeSN, infoSN, ",
        "groupid, pubid, codeCrc, ",
        "codeContext,packCount,packLength)",
        "values (#{infoCodeSN,jdbcType=INTEGER}, #{infoSN,jdbcType=INTEGER}, ",
    ...
    @Select({
        "select",
        "infoCodeSN, infoSN, groupid, pubid, codeCrc, codeContext,packCount,packLength",
        "from t_infocode",
        "where infoSN = #{infoSN,jdbcType=INTEGER}"
    })
    @Results({
        @Result(column="infoCodeSN", property="infoCodeSN", jdbcType=JdbcType.INTEGER, id=true),
    ...
    @Update({
        "update t_infocode",
        "set infoSN = #{infoSN,jdbcType=INTEGER},",
          "pubid = #{pubid,jdbcType=INTEGER},",
          "groupid = #{groupid,jdbcType=INTEGER},",
          "codeCrc = #{codecrc,jdbcType=INTEGER},",
