...
public interface playlistcodeMapper {
	@Delete({ "delete from t_playlistcode", "where playlistCodeSN = #{playlistCodeSN,jdbcType=INTEGER}" })
	int deleteByPrimaryKey(Integer playlistCodeSN);

	@Delete({ "delete from t_playlistcode", "where groupid = #{0}" })
	int deleteBygroupid(Integer groupid);

	@Delete({ "delete from t_playlistcode", "where playlistSN = #{playlistSN,jdbcType=INTEGER}" })
	...
	@Insert({ "insert into t_playlistcode (playlistCodeSN, groupid, containADID, lifeAct, lifeDie, playlistSN, ",
			"pubid, playlistCrc, ", "codeContext,packCount,packLength)",
	...
	@Select({ "select",
			...
			"from t_playlistcode", "where playlistcodeSN = #{playlistcodeSN,jdbcType=INTEGER}" })
	@Results({ @Result(column = "playlistCodeSN", property = "playlistCodeSN", jdbcType = JdbcType.INTEGER, id = true),
	...
	@Select({ "select", "pubid", "from t_playlistcode", "where playlistSN = #{playlistSN,jdbcType=INTEGER}" })
	@Result(column = "pubid", property = "pubid", jdbcType = JdbcType.INTEGER)
	...
	@Update({ "update t_playlistcode", "set playlistSN = #{playlistsn,jdbcType=INTEGER},",
