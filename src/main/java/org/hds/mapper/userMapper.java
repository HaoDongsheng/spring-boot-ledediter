...
public interface userMapper {
	@Delete({ "delete from t_user", "where adminID = #{adminid,jdbcType=INTEGER}" })
	int deleteByPrimaryKey(Integer adminid);

	@Delete({ "delete from t_user", "where projectid = #{0}" })
	int deleteByprojectid(String projectid);

	@Insert({ "insert into t_user (adminID, adminName, ", "adminPwd, adminStatus, ", "expDate, adminPermission, ",
			"projectid, groupids, adminLevel, ", "parentId, inherit, ", "GrpCount, adminCount, ",
			"isSuperuser, DelIndex, adminRemarks)",
			...
			"#{issuperuser,jdbcType=INTEGER}, #{delindex,jdbcType=INTEGER}, #{adminRemarks,jdbcType=VARCHAR})" })
	...
	@Select({ "select", "adminID, adminName, adminPwd, adminStatus, expDate, adminPermission, projectid, groupids,",
			"adminLevel, parentId, inherit, GrpCount, adminCount, isSuperuser, DelIndex, adminRemarks", "from t_user",
			"where adminID = #{adminid,jdbcType=INTEGER}" })
	@Results({ @Result(column = "adminID", property = "adminid", jdbcType = JdbcType.INTEGER, id = true),
			...
			@Result(column = "adminRemarks", property = "adminRemarks", jdbcType = JdbcType.VARCHAR) })
	...
	@Select({ "select", "count(*)", "from t_user", "where adminName = #{0} and DelIndex=0" })
	int selectCountByName(String adminName);

	@Select({ "select", "count(*)", "from t_user", "where adminName = #{0} and adminID <> #{1}" })
	...
	@Update({ "update t_user", "set adminName = #{adminname,jdbcType=VARCHAR},",
			"adminPwd = #{adminpwd,jdbcType=VARCHAR},", "adminStatus = #{adminstatus,jdbcType=INTEGER},",
			"expDate = #{expdate,jdbcType=VARCHAR},", "adminPermission = #{adminpermission,jdbcType=VARCHAR},",
			"projectid = #{projectid,jdbcType=VARCHAR},", "groupids = #{groupids,jdbcType=VARCHAR},",
			"adminLevel = #{adminlevel,jdbcType=INTEGER},", "parentId = #{parentid,jdbcType=INTEGER},",
			"inherit = #{inherit,jdbcType=INTEGER},", "GrpCount = #{grpcount,jdbcType=INTEGER},",
			"adminCount = #{admincount,jdbcType=INTEGER},", "isSuperuser = #{issuperuser,jdbcType=INTEGER},",
			"DelIndex = #{delindex,jdbcType=INTEGER},", "adminRemarks = #{adminRemarks,jdbcType=INTEGER},",
			"where adminID = #{adminid,jdbcType=INTEGER}" })
	int updateByPrimaryKey(user record);

	@Update({ "update t_user", "set groupids = CONCAT(groupids,#{1})", "where adminName = #{0}" })
	int updateGroupidsByname(String adminName, String grpid);

	@Update({ "update t_user", "set groupids = CONCAT(groupids,#{1})", "where projectid = #{0}" })
	int updateGroupidsByprojectid(String projectid, String grpid);
}