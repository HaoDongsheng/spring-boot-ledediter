...
public interface advertisementMapper {
	@Delete({ "delete from t_advertisement", "where infoSN = #{infoSN,jdbcType=INTEGER}" })
	int deleteByPrimaryKey(Integer infoSN);

	@Insert({ "insert into t_advertisement (infoSN, advName, ",
			"groupid, lifeAct, lifeDie, advType, infoState, creater, createDate,", "playMode, pubid, playTimelength, ",
			"delindex, BackgroundStyle)", "values (#{infoSN,jdbcType=INTEGER}, #{advname,jdbcType=VARCHAR}, ",
	...
	@Select({ "select",
			...
			"from t_advertisement", "where infoSN = #{infoSN,jdbcType=INTEGER}" })
	...
	@Select({ "select", "deleteDate", "from t_advertisement", "where infoSN = #{infoSN,jdbcType=INTEGER}" })
	@Result(column = "deleteDate", property = "deleteDate", jdbcType = JdbcType.VARCHAR)
	Object selectdeleteDateByPrimaryKey(Integer infoSN);

	@Select({ "select", "publishDate", "from t_advertisement", "where infoSN = #{infoSN,jdbcType=INTEGER}" })
	@Result(column = "publishDate", property = "publishDate", jdbcType = JdbcType.VARCHAR)
	...
	@Update({ "update t_advertisement", "set advName = #{advname,jdbcType=VARCHAR},",
			...
			"delindex = #{delindex,jdbcType=INTEGER},", "BackgroundStyle = #{backgroundstyle,jdbcType=LONGVARCHAR}",
