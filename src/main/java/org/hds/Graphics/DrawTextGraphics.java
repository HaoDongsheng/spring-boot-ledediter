...
public class DrawTextGraphics {
	//图文变成图片
	public static BufferedImage getImagebyJsonArray(int playtype,JSONArray JarrContext,JSONObject jsonspecial,int itemwidth,int itemheight) {
		try {
			int width = 0,left = 0;
						...
						{//图文混排数据
							...
							{				
								...
								if(jsonspecial!=null || !isIncode)
								{
							        ...
							        txth = (int)(txth * sy);
							        if(itemheight>txth){ top = (itemheight - txth)/2;}
						...
						{//图文混排数据
							        	...
							        	{	
							        		if(JSONlist.size()>0)
							        		{
							        			...
							        			if(jObject.getIntValue("type")==0)
							        			{
							        				...
							        				JSONlist.get(JSONlist.size() - 1).put("w", (left + charw)/itemwidth * itemwidth - px);
							        			}
							        		}
							...
							{										
								...
								{							
							        ...
							        if(left/itemwidth!= (left + imgWidth)/itemwidth)
						        	{	
					...
					for(int i=0;i<JSONlist.size();i++)
					{
						...
						if(type==0)
						{
					        ...
					        if(backcolor!="#ffffff")
					        {
						        ...
						        graphics.fillRect(jsonObject.getIntValue("x"), 0, jsonObject.getIntValue("w"), itemheight);
					        }
					        ...
					        if(jsonspecial!=null)
					        {
					        	...
					        	if(jsonsketch!=null)
					        	{
					        		Color m_sketchcolor=new Color(Integer.parseInt(jsonsketch.getString("strokeStyle").substring(1),16));   					          	                		
					        	...
					        	if(jsonshadow!=null)
					        	{
					        		Color m_shadowcolor=new Color(Integer.parseInt(jsonshadow.getString("shadowColor").substring(1),16));   					          	                		
					        	...
					        	if(jsongradient!=null)
					        	{
					        		...
					        		Color gradientcolor1=new Color(Integer.parseInt(jsongradient.getString("gradientcolor1").substring(1),16));
					        		Color gradientcolor2=new Color(Integer.parseInt(jsongradient.getString("gradientcolor2").substring(1),16));        		
					        		switch (gradientdirection) {
									case "horizontal":{paint = new GradientPaint(0,0,gradientcolor1,width, 0,gradientcolor2,true);};break;
									case "vertical":{paint = new GradientPaint(0,0,gradientcolor1,0, itemheight,gradientcolor2,true);};break;
									case "oblique":{paint = new GradientPaint(0,0,gradientcolor1,width, itemheight,gradientcolor2,true);};break;
									}
					        ...
					        if(paint==null)
					        {graphics.setColor(m_forecolor);}// 在换成黑色            
							...
							graphics.drawImage(img, jsonObject.getIntValue("x"), jsonObject.getIntValue("y"), img.getWidth(), img.getHeight(),null);
						}						
												
					}					
					//ImageIO.write(bmp, "PNG", new File("C:/11.png"));
					//return null;
					return bmp;
				}
				else {
	...
	public static BufferedImage getbufImage(String txtval,String fontName,int fontSize,String backcolor,String forecolor,int itemheight,JSONObject jsonspecial) {
		              
		try {					
		if(forecolor=="" || forecolor.equals("")){forecolor="#000000";}
		if(backcolor=="" || backcolor.equals("")){backcolor="#ffffff";}
    ...
    public static byte[] Gif2Gif(byte[] imgBytes, int w, int h) {
		try {
	        ...
	        GifImage resizeImage = GifTransformer.resize(srcImage, w, h, false);
	        GifEncoder.encode(resizeImage, os);
            ...
            for (int i = 0; i < n; i++) {
                BufferedImage frame = decoder.getFrame(i); //得到帧
                int delay = decoder.getDelay(i); //得到延迟时间                 
                //ImageIO.write(frame, "png", new File("E:/or"+i+".png"));
                
                BufferedImage newImage = getnewbufImage(frame,w,h);     
                                                
                //float quality = (float)0.5;
                //BufferedImage nImage = zoomBufferedImageByQuality(newImage,quality);
                
                //ImageIO.write(newImage, "png", new File("E:/"+i+".png"));
